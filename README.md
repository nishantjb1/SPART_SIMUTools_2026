...
1. **Temporal eligibility (muting)**: vehicles outside the ego's detection range are deferred using a linear closing-velocity estimate. Muted vehicles are skipped entirely — no segment building, no kernel call. Achieves ~52% muting on the INTERACTION intersection.
